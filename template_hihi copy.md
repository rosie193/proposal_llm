...
**Divide modules into:**
- **Standard AI Modules**: Available in KB, can reuse existing slides
- **Custom Modules**: Need to create new descriptions and images

**Source**: S2 - "List of VA use cases" → Search KB "DOCUMENT" or "viAct_Proposal" for same use case to determine if standard or custom

### 7.2 Standard AI Modules Template

| Content | Source/Guidance |
|---------|------------------|
| **Standard AI Modules (from KB)** | **Input**:<br>+ Standard AI Modules name (from S2 - "List of VA use cases")<br>**Process**:<br>1. Search KB "DOCUMENT" or "viAct_Proposal" for same use case<br>2. Copy the same Slide AI modules Use case from KB<br>3. If client has footage → Insert image to slide<br>**Output**:<br>+ Insert available slides from KB<br>**Reference**: KB "DOCUMENT" → Search for module name or use case |

**Module Template (All modules follow this structure):**
...
• Alert Trigger Logic: When does it alert? 
  Example: "Alert triggers when a person crosses line A"
  Example: "Alert triggers when PPE is not detected"

• Client Data Requirements: Images, video samples, or labels required for training (if custom module)

• Preconditions: Camera angle, height, lighting requirements.
  Example: "Camera must maintain a suitable distance for clear observation of object, typically between 5 to 10 meters"
```

### 7.3 Custom Modules Template

| Content | Source/Guidance |
|---------|------------------|
| **Customize Modules** | **Input**:<br>+ Description of Customize Modules (from S2 - "Custom AI use cases")<br>**Process**:<br>1. Search KB for similar use cases (may not find exact match)<br>2. Create use case image with description from outline<br>3. Use Gemini/Veo3 to create image/video if needed<br>4. Insert image/video in slide and draw red bounding box around violation area<br>5. Insert the Slide content, include:<br>   + Purpose: [What it detects]<br>   + Alert: [When it alerts]<br>   + Request: [Provide object types image for model training]<br>   + Pre-condition: Camera must maintain a suitable distance for clear observation of object, typically between ... to ... meters (5 to 10 meters)<br>**Output**:<br>+ A slide with description and image |
